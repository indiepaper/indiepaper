defmodule IndiePaper.Authors.AuthorToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  defp build_hashed_token(author, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %IndiePaper.Authors.AuthorToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       author_id: author.id
  ...
  def token_and_context_query(token, context) do
    from IndiePaper.Authors.AuthorToken, where: [token: ^token, context: ^context]
  end
  ...
  def author_and_contexts_query(author, :all) do
    from t in IndiePaper.Authors.AuthorToken, where: t.author_id == ^author.id
  end
