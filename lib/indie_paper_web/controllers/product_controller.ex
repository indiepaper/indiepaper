defmodule IndiePaperWeb.ProductController do
  ...
  def new(conn, %{"book_slug" => book_slug}) do
    book = Books.get_book!(book_slug) |> Books.with_assoc(:assets)
    changeset = Products.change_product(%Products.Product{})
    render(conn, "new.html", changeset: changeset, book: book)
  end

  def create(conn, %{"book_slug" => book_slug, "product" => product_params}) do
    book = Books.get_book_from_slug!(book_slug) |> Books.with_assoc(:assets)
  ...
  def edit(conn, %{"book_slug" => book_slug, "id" => product_id}) do
    book = Books.get_book_from_slug!(book_slug) |> Books.with_assoc(:assets)
  ...
  def update(conn, %{"book_slug" => book_slug, "id" => product_id, "product" => product_params}) do
    book = Books.get_book_from_slug!(book_slug) |> Books.with_assoc(:assets)
    product = Products.get_product!(product_id)
