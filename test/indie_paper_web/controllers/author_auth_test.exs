defmodule IndiePaperWeb.AuthorAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, IndiePaperWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_author/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      IndiePaperWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_authenticated_author/2" do
    test "redirects if author is not authenticated", %{conn: conn} do
      ...
      assert get_flash(conn, :info) =~ "You should be signed in"
    end
